...
from .libdiamond import version, run_blastp
...
def _run_blast(inputfile, outputfile, database, processors):
    ''' runs protein BLAST and saves results '''
    print('blasting %s vs %s | Diamond %s'%(input_file,database, version()))
    args = ['-p', processors, '-d', database, '-q', inputfile, '-o', outputfile, '--more-sensitive', '--max-target-seqs', '1', '--quiet']
    run_blastp(*args)
...
if __name__ == "__main__":
    ...
    if file_type == 1:
        ...
        _run_blast(input_file, blast_results, kegg_prot_db, str(processors))
    elif file_type == 2:
